...
public class FoodCartMap {
	...
	public List<FoodTruckData> 
	showCloseByTrucks(List<FoodTruckData> trucks, double limit) {
		if (userPos == null) {
			return trucks;
		}
		List<FoodTruckData> closeByTrucks = new ArrayList<FoodTruckData>(); 
		for (FoodTruckData truck : trucks) {
			if (distanceFromUser(truck) < limit) {
				closeByTrucks.add(truck);
			}
		}
